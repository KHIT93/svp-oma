    ...
    def handle(self, *args, **options):
        ...
        for windturbine in WindTurbine.objects.all():
            if windturbine.ip_address == "0.0.0.0":
                print("Windturbine " + str(windturbine) + " skipped due to missing IP-address")
            else:
                print("Running data sync for windturbine " + str(windturbine) + " at " + windturbine.ip_address)
                try:
                    r = requests.get('http://' + windturbine.ip_address + '/windturbinedata/all/')
                    if r.status_code == 200:
                        print(r)
                except:
                    print("The windturbine " + str(windturbine) + " at " + windturbine.ip_address + " could not be reached")
