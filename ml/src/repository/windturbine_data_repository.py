...
class WindturbineDataRepo(object):
	...
	def get_new(self, windturbine_id):
		...
		self.windturbine_data = self.windturbine_data
		self.last_record = self.windturbine_data[0][1]
		...
		self.windturbine_data = np.average(self.windturbine_data[:,order], axis=0)
		np.insert(self.windturbine_data, 1, mean_datetime)
